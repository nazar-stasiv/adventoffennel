...
(fn count-group-answers [input]
  ...
  (let [res []
    ...
    (each [_ line (ipairs input)]
      (if (= "" line)
          (do (table.insert res (length (lume.unique (aoc.table-flatten group))))
              (aoc.table-reset group))
          (table.insert group (aoc.string-toarray line))))
    res))

(fn solve [input]
  (let [group (count-group-answers input)]
    (accumulate [sum 0 _ g (ipairs group)]
      (+ sum g))))
