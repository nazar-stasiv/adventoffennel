...
(fn real-input [xs]
  (let [locations []
        seeds (table.unpack (lume.map (aoc.table-range xs 1 1) #(aoc.string-tonumarray $)))
...
(fn read-matrix [lines]
  (let [matrix []]
    (each [_ line (ipairs lines)]
      (table.insert matrix (aoc.string-toarray line)))
    matrix))
...
(real-input (expand-empty-space (read-matrix (aoc.string-from "2023/11.inp"))))
#+end_src
