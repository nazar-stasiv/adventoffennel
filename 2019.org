...
(fn some-adjacent? [xs]
  "true if there are at least two adjacent equal digits"
  (var res false)
  (for [i 2 (length xs) &until res]
    (when (= (. xs i)
             (. xs (- i 1)))
      (set res true)))
  res)
...
(fn never-decrease? [xs]
  "true if all digits GE previous adjacent digit"
  (var res true)
...
#+begin_src fennel :session day04 :exports both
(fn two-adjacent? [xs]
  "true if there is at least one pair of equal digits"
  (let [bins (aoc.partition-by xs #(= $1 $2))]
